<template>
  ...
  // Assuming the user is logged in and userId is retrieved from the session or token
  const userId = 1; // You can replace this with logic to get the logged-in user's ID
  ...
  onMounted(async () => {
    try {
      ...
      const { data } = await $axios.get(`/api/dashboard/${userId}`); // API call to the correct endpoint
